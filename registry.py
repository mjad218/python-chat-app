            ...
            try:
                ...
                #   SEARCH  #
                    ...
                    # if an account with this username is not exist, and the account is created
                        ...
                        db.room_create(message[1], message[2])
                        response = "room-creation-success"
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response) 
                        self.tcpClientSocket.send(response.encode())

                #adding user to the room 
                elif message[0] == "ADDUSER":
                    #check if the user is already in the room

                    print(message[0], message[1], message[2] )


                    if db.isUserAlreadyInRoom(message[1], message[2]):
                        response = "user-already-exists"
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                        self.tcpClientSocket.send(response.encode())
                        
                    elif not db.is_room_exist(message[1]) or not db.is_account_exist(message[2]):
                        response = "wrong-info"
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                        self.tcpClientSocket.send(response.encode())                        
                        ...
                        db.add_client(message[1], message[2])
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                        self.tcpClientSocket.send(response.encode())                        
...
class UDPServer(threading.Thread):
    ...
    def resetTimer(self):
        self.timer.cancel()
        self.timer = threading.Timer(6, self.waitHelloMessage)
        self.timer.start()
...
while inputs:
    ...
    for s in readable:
        ...
        if s is tcpSocket:
            ...
            list_of_clients.append(tcpClientSocket)
