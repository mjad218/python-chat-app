...
class peerMain:
    ...
    def __init__(self):
        ...
        while choice != "3":
            ...
            # to enter the username of the user that is wanted to be chatted
                ...
                self.createRoom(roomid,self.loginCredentials[0])
                
            elif choice == "7" and self.isOnline:
                roomid = input("Enter room id: ")
                username = input("Enter username: ")
                self.addUsers(roomid, username)
    ...
    #Creating new room    
    def createRoom(self,roomid,peerUsername):
        message = "CREATEROOM " + roomid + " " + peerUsername
        self.tcpClientSocket.send(message.encode())
        logging.info("Send to " + self.registryName + ":" + str(self.registryPort) + " -> " + message)
        response = self.tcpClientSocket.recv(1024).decode().split()
        logging.info("Received from " + self.registryName + " -> " + " ".join(response))
        if response[0] == "room-creation-success":
            print("Room created successfully! \n")
        elif response[0] == "room-exist":
            print("Room already exists!\n")
            
    
    #adding users to room
    def addUsers(self, roomid, peerUserName):
        message = "ADDUSER " + roomid + " " + peerUserName
        self.tcpClientSocket.send(message.encode())
        logging.info("Send to " + self.registryName + ":" + str(self.registryPort) + " -> " + message)
        response = self.tcpClientSocket.recv(1024).decode().split()
        print(response)
        ...
        elif response[0] == "user-already-exists":
            print("user already exists\n")
        elif response[0] == "wrong-info":
            print("wrong room id or wrong username\n")
    ...
    def sendHelloMessage(self):
        ...
        self.timer = threading.Timer(5, self.sendHelloMessage)
        self.timer.start()
